requirejs(['algoliaBundle'], function(algoliaBundle) {
	algoliaBundle.$(function ($) {
		...
		function handleInputCrossAutocomplete(input) {
			var searchButton = input.closest('#algolia-searchbox').find('button.search');
			var searchButtonDisabled = (input.val().length > 0) ? false : true;
			input.removeClass('search-text-exist')
			if(input.val().length){
				input.addClass('search-text-exist')
			}
		...
		if($(algoliaConfig.autocomplete.selector).length){
